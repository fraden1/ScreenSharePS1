...
Write-Host -ForegroundColor Red "###        ###    ### ###     ### #########  ########## ###    #### #######       ########   ######## ";
Write-Host "";
Write-Host -ForegroundColor Red "BAM PowerShell - github.com/fraden1 " -NoNewLine
Write-Host "";

function Test-Admin {;$currentUser = New-Object Security.Principal.WindowsPrincipal $([Security.Principal.WindowsIdentity]::GetCurrent());$currentUser.IsInRole([Security.Principal.WindowsBuiltinRole]::Administrator);}
if (!(Test-Admin)) {
    Write-Warning "Run This With Administrator Privilege"
    Start-Sleep 10
    Exit
}
...
$Bam | Out-GridView -PassThru -Title "BAM PowerShell - github.com/fraden1 - BAM key entries $($Bam.count)  - User TimeZone: ($UserTime) -> ActiveBias: ( $Bias) - DayLightTime: ($Day)"
